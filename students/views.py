...
from professors.models import Courses,CourseList,Announcements,Content,Evals,Branch
...
def course_detail(request,pk,prof=False):
    ...
    if prof:
        ...
        if Courses.objects.filter(~Q(grade=None),course = selected_course).exists():
            stu_grade = True
        else:
            stu_grade = False
    else:
...
def course_registration(request):
    ...
    enrolled_courses=Courses.objects.filter(student=curr_student,date_added=todays_date)

    for cdc in cdcs:
        try:
            Courses.objects.create(course=cdc,student=curr_student,marks=0)
...
def add_enrolled_course(request,pk):
    ...
    Courses.objects.create(course=selected_course,student=curr_student,marks=0)
